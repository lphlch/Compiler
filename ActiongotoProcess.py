...
ACTION_GOTO = (
    []
)  # ACTION_GOTO表，ACTION_GO[状态i][符号j]，0为acc，x为移进，-x为用产生式x规约（这个产生式下标为accept_grammar_point的下标号
    ...
    try:
        ...
        if curr in First_point:
            for item in First_point[curr]:
                if (
                    item not in CLOSURE[y]
                    and accept_grammar_point[item]["forward"] in curr_fir
                ):
                    CLOSURE[x].add(item)
                    CLOSURE[y].add(item)
...
def GET_PER_CLOSURE():
    ...
    for i, item in zip(range(len(accept_grammar_point)), accept_grammar_point):
        ...
        if (
            item["origin"] == 0 and item["forward"] == "#" and item["point"] == 0
        ):  # 0号项目集
        ...
        for k in CLOSURESET[i]:
            ...
            if item["finish"] == True:
                if item["forward"] in ACTION_GOTO[i]:
                    print(
                        "Warning:,%d号项目集族的\t %s \t符号与产生式" % (i, item["forward"]),
                        accept_grammar_point[k],
                        "冲突！",
                    )
                    print("项目集族见下：")
